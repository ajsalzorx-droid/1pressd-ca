...
export function MenuGrid(){
 ...
 return <section id="full-menu" className="fullMenu" aria-labelledby="full-menu-title">
  <div className="menuTop"><div><p className="kicker">The full line-up</p><h2 id="full-menu-title">THE PRESS’D<br/>MENU.</h2></div><div className="menuTopAside"><span>Fresh every day</span><p>Wellness food, specialty coffee and feel-good favourites—made for here or ordered through WhatsApp.</p></div></div>
  ...
  <div className="menuCategoryStatus" aria-live="polite"><span>{category}</span><small>{shown.length.toString().padStart(2,"0")} selections</small></div>
  <motion.div layout className="menuGrid"><AnimatePresence mode="popLayout">{shown.map(item=><motion.article layout initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:.97}} transition={{duration:.45,ease:[.16,1,.3,1]}} key={item.name} data-category={item.cat} onClick={()=>setSelected(item)} tabIndex={0} onKeyDown={e=>{if(e.key==="Enter")setSelected(item)}}><div className="menuPhoto"><Image src={item.img} fill sizes="(max-width:600px) 94vw, (max-width:1000px) 48vw, 46vw" alt={item.name}/>{item.tag&&<b>{item.tag}</b>}</div><div className="menuInfo"><h3>{item.name}</h3><strong>{item.price}</strong><p>{item.desc}</p><small>{item.protein} · {item.kcal}</small><button className="addCart" onClick={e=>{e.stopPropagation();changeQty(item.name,1)}}><Plus size={15}/> Add to cart {cart[item.name]?`(${cart[item.name]})`:""}</button></div></motion.article>)}</AnimatePresence></motion.div>
