...
export default function Home(){
 const [open,setOpen]=useState(false); const [loading,setLoading]=useState(true); const {scrollYProgress}=useScroll(); const heroY=useTransform(scrollYProgress,[0,.2],[0,90]);
 useEffect(()=>{document.body.style.overflow=loading||open?"hidden":"";return()=>{document.body.style.overflow=""}},[loading,open]);
 return <><AnimatePresence>{loading&&<motion.div className="loader" initial={{opacity:1}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.7,ease:"easeInOut"}}><video src="/pressd-loader.mp4" autoPlay muted playsInline preload="auto" onEnded={()=>setLoading(false)} onError={()=>setLoading(false)} /><button onClick={()=>setLoading(false)}>Skip intro</button><div className="loaderMark"><Logo light/><span>Loading your good day</span></div></motion.div>}</AnimatePresence><main>
  ...
  <header className="nav"><Logo light/><nav>{["Menu","Pet Friendly","Work at PRESS’D","Wellness","About","Visit"].map(x=><a href={`#${x.split(" ")[0].toLowerCase()}`} key={x}>{x}</a>)}</nav><Btn href="#visit">Book a table</Btn><button className="menuBtn" onClick={()=>setOpen(!open)} aria-label="Menu" aria-expanded={open}>{open?<X/>:<Menu/>}</button></header>
  ...
  <aside className="heroRail" aria-label="Quick links">
    <a href="#menu"><Image src="https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=900&q=86" fill sizes="28vw" alt="Healthy PRESS’D menu"/><span>Menu <ArrowRight size={17}/></span></a>
    <a href="#visit"><Image src="https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=900&q=86" fill sizes="28vw" alt="Coffee and table at PRESS’D"/><span>Book a table <ArrowRight size={17}/></span></a>
    <a href="#about"><Image src="https://images.unsplash.com/photo-1554118811-1e0d58224f24?auto=format&fit=crop&w=900&q=86" fill sizes="28vw" alt="PRESS’D café interior"/><span>Our café <ArrowRight size={17}/></span></a>
  </aside>
  ...
  <a className="stickyMenu" href="#menu">View menu <ArrowRight size={15}/></a>
 </main></>
}
